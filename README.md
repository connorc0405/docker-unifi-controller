...
- TLSv1.0 and SSLHello are disabled.  Only TLSv1.2 is enabled.
- Supports user certificates. See docker-compose file and cli arguments for guidance.
...
services:
  unifi-controller:
    ...
    volumes:
      - <path to data>:/config
      - <path to certs>:/certs  # Needs fullchain.pem and privkey.pem
    ports:
      - 8443:8443
      - 3478:3478/udp
      - 10001:10001/udp
...
docker run -d \
  ...
  -v <path to certs>:/certs \  # Must include fullchain.pem and privkey.pem
  --restart unless-stopped \
